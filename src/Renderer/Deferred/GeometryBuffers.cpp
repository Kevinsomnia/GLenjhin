...
GeometryBuffers::GeometryBuffers(uint32_t width, uint32_t height, uint8_t depth) : m_Width(width), m_Height(height)
{
    ...
    m_NormalSmoothGBuffer = new Texture2D(width, height, TextureFormat::RGBAHalf, /*readable=*/ false);
    ...
    m_EmissionOcclGBuffer = new Texture2D(width, height, TextureFormat::RGBAHalf, /*readable=*/ false);
